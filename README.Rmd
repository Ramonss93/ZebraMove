---
title: "Can animal movement and remote sensing data help to improve conservation efforts?"
subtitle: "A case study on plains and Grevy’s zebras"
author: "Matthias F. Biber, Global Change Ecology, University of Bayreuth, Bayreuth, Germany"
output:
  github_document:
    toc: true
    toc_depth: 2
...
Since the beginning of the Industrial Revolution anthropogenic activities have an increasing effect on the global environment. By now these effects are so profound, that the term "Anthropocene" has been introduced to describe the current ecological epoch [@Crutzen2002; @Steffen2007; @Lewis2015; @Corlett2015]. Fossil fuel emissions and land use change have caused an increase in carbon dioxide (CO~2~) concentrations by 40% compared to pre-industrial levels. The increase in atmospheric CO~2~ is the largest driver of climate change. Global surface temperature is predicted to be more than 1.5°C higher by the end of the 21^st^ century relative to the average global surface temperature between 1850 and 1990 [@IPCC2013]. Anthropogenic activities have not only affected the Earth's climate system, but have also considerably changed the world's ecosystems. 
...
summary <- read.csv("Results/summary.csv")[,-1]
colnames(summary) <- c("ID", "Start date", "End date", 
                         "Duration (days)", "Distance (km)", 
                         "Time interval (min)")
pander::pandoc.table(summary, split.table = Inf, missing="-")
```
...
$$ EVI = 2.5\times(NIR - RED)/(NIR + 6.0\times RED - 7.5\times BLUE + 1.0) $$
...
$$ AWEI_{sh} = BLUE + 2.5\times GREEN - 1.5\times(NIR + SWIR1) - (0.25\timesSWIR2) $$
...
```{r, results="asis", echo=FALSE}
summary_wdpa <- read.csv("Results/summary_wdpa.csv")[,-1]
colnames(summary_wdpa) <- c("Name", "Designation", "IUCN Category", 
                              "Area (km2)", "Year")
pander::pandoc.table(summary_wdpa, digits=0, split.table=Inf, missing="-")
```
...
```{r, results="asis", echo=FALSE}
lcov_pas_df <- read.csv("Results/lcov_pas_df.csv")[,-1]
colnames(lcov_pas_df) <- c("", "Chobe", "Makgadikgadi Pans", "Maun", "Moremi", "Nxai Pan", "Okavango Delta", "Okavango Delta System", "Non-protected")
pander::pandoc.table(lcov_pas_df, digits=1, split.table=Inf, missing="-")
```
...
```{r, results="asis", echo=FALSE}
migration <- read.csv("Results/migration.csv")[,-1]
colnames(migration) <- c("ID", "Direction", "Start Date", 
                         "End Date", "Duration (days)", "Distance (km)")
pander::pandoc.table(migration, split.table=Inf, missing="-")
```
...
**Fig. 7.** Absolute turning angle (°) and speed (m/s) for each migration state (Northern Range, Southern Migration, Southern Range and Northern Migration).
...
```{r, results="asis", echo=FALSE}
pValues <- read.csv("Results/pValues.csv")[,-1]
pander::pandoc.table(pValues, digits=3, split.table=Inf, missing="-")
```
...
```{r, results="asis", echo=FALSE}
summary <- read.csv("Results/summary_zebra_move.csv")[,-1]
colnames(summary) <- c("ID", "Start date", "End date", "Duration (days)", "Distance (km)", "Time interval (min)")
pander::pandoc.table(summary)
```
...
```{r, results="asis", echo=FALSE}
luse_area <- read.csv("Results/luse_area.csv")[,-1]
names(luse_area) <- c("Land use", "Area", "Percentage (%)")
pander::pandoc.table(luse_area[,c("Land use", "Percentage (%)")], digits=2)
```
...
```{r, results="asis", echo=FALSE}
summary_wdpa <- read.csv("Results/summary_wdpa.csv")[,-1]
colnames(summary_wdpa) <- c("Name", "Designation", "IUCN Category", "Area (km2)", "Year")
pander::pandoc.table(summary_wdpa, digits=0, split.table=Inf)
```
